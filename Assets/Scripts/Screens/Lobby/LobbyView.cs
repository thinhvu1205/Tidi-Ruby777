...
public class LobbyView : BaseView
{
    ...
    GameObject objDot, btnEx, gameItemObject, modelLobby, iconSafe, btnSafe, btnGiftCode, btnLeaderboard,
        btnSupport, objBot, icNotiMail, icNotiFree, icNotiMessage, bannerTemp, btnBannerNews;
    ...
    public void setQuickPlayGame(int gameID)
    {
        ...
        lbQuickGame.text = Globals.Config.getTextConfig(gameID.ToString());
    ...
    void onClickGame(ItemGame itemGame)
    {
        ...
        {
            Debug.Log(" Dang Click Game ròi " +
             isClicked + ", " + (User.userMain.lastGameID != 0) + ", " + (User.userMain.lastGameID != itemGame.gameID)
             + User.userMain.lastGameID + ", " + itemGame.gameID);
            return;
        }
        isClicked = true;
        ...
        if (Config.curGameId != (int)GAMEID.SLOT_SIXIANG)
        {
            ...
            Globals.Config.isSendingSelectGame = true;
        }
        else
        {
