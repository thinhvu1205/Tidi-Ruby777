...
public class BaseSlotGameView : GameView
{
    ...
    public virtual void onClickSpin()
    {
        ...
        else
        {
            if (animBgFreeSpin != null)
            {
                ...
                if (Globals.Config.curGameId == (int)Globals.GAMEID.SLOT_INCA)
                {
        ...
        if (!isInFreeSpin && isFreeSpin)
        {
            ...
            if (lbStateWin != null)
            {
                lbStateWin.text = Globals.Config.getTextConfig("txt_total_win");
            }
            else
            {
        ...
        else
        {
            ...
            if (lbStateWin != null)
            {
                lbStateWin.text = Globals.Config.getTextConfig("txt_last_win");
            }
            else
            {
    ...
    public virtual void onStopSpin() //dung het cot cuoi cung.bat dau check data show result c?c th?
    {
        ...
        if (!isInFreeSpin) //khong co freespin
        {
            ...
            if ((long)finishData["agWin"] != 0)
            {
                ...
                if (lbStateWin != null)
                {
                    lbStateWin.text = Globals.Config.getTextConfig("txt_win");
                }
                else
                {
        ...
        else if (isInFreeSpin && isFreeSpin == false) //freespin turn cuoi cung
        {
            ...
            TweenCallback acShowTotalWinFreeSpin = () =>
            {
                ...
                if (lbStateWin != null)
                {
                    Debug.Log("Chayy vao acShowTotalWinFreeSpin");
                    lbStateWin.text = Globals.Config.getTextConfig("txt_total_win");
                }
                else
                {
            ...
            if (countTotalAgFreespin > payLines.Count * totalListBetRoom[currentMarkBet])
            {
                winType = 1;
            }
        ...
        TweenCallback acCheckNextSpin = () =>
        {
            ...
            Debug.Log("action check next Spin:spintype=" + spintype);
            setSpinType();
            ...
            {
                Debug.Log("action check next Spin:isFreeSpin=" + isInFreeSpin);
                resetSlotView();
                setStateBtnSpin();
            }
        };
        TweenCallback acShowOneWinLine = () =>
        {
            ...
            Debug.Log("acShowOneWinLine:" + isFreeSpin);
            if (!isFreeSpin)
            {
                lbCurrentChips.setValue((long)finishData["AG"], true);
                refreshBetRoomAndMarkbet();
        ...
        if (winningLines.Count > 0 || getInt(finishData, "agWin") > 0) //co line win va quay thuong thi moi show an tung line
        {
            ...
            {
                //Globals.Config.tweenNumberToNumber(lbCurrentChips, (long)finishData["AG"], agPlayer, 0.5f);
                Debug.Log("acShowAnimChipBay111");
                lbCurrentChips.setValue(getLong(finishData, "AG"), true);
                refreshBetRoomAndMarkbet();
                showAnimChipBay();
                handleActionResult();
    ...
    protected virtual void showBigWin()
    {
        ...
        {
            Globals.Config.tweenNumberTo(lbBigWin, getInt(finishData, "agWin"), 0, 2.0f);
        }
    ...
    protected virtual void showMegaWin()
    {
        ...
        if (isInFreeSpin == true && isFreeSpin == false) //vua quay het freespin turn cuoi cung;
        {
            Globals.Config.tweenNumberTo(lbBigWin, countTotalAgFreespin, 0, 2.0f);
        }
        else
        {
            Globals.Config.tweenNumberTo(lbBigWin, getInt(finishData, "agWin"), 0, 2.0f);
    ...
    protected virtual void showFreeSpin()
    {
        ...
        animEffect.AnimationState.SetAnimation(0, "cam", false);
        effectAnimEndListenter = () =>
        {
            effectContainer.SetActive(false);
            handleActionResult();
    ...
    public void nextColStop()
    {
        ...
        //{
        if (countScatter >= 2 && currentIndexStop == indexCheck3rdScatter - 1 && Globals.Config.curGameId != (int)Globals.GAMEID.SLOT_JUICY_GARDEN)
        {
            DOTween.Sequence()
                .AppendInterval(2.0f)
                .AppendCallback(() =>
    ...
    public virtual void changeBetRoom(string type)
    {
        ...
        if (currentMarkBet == listBetRoom.Count - 1)
        {
            lbStateBet.text = Globals.Config.getTextConfig("txt_max_bet");
        }
        else
        {
            lbStateBet.text = Globals.Config.getTextConfig("txt_bet");
        }
    }
    ...
    {
        //Globals.Logging.Log("setSpinType freespinLeft ==" + freespinLeft);
        if (freespinLeft > 0)
        {
