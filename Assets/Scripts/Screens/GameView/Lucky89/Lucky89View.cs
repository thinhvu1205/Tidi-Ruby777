...
public class Lucky89View : GameView
{
    ...
    private bool _isMyDrawTime, _drawACard;
    ...
    private void _HandleStartGame(JObject data)
    {
        ...
        foreach (Player player in players) player.setTurn(true, (float)data["T"] / 1000);
    ...
    private void _HandleAnyoneBets(JObject data)
    {
        ...
        ((PlayerViewLucky89)player.playerView).ShowHideBetChips(true, (int)data["AG"]);
    ...
    private void _HandleReceiveMyCards(JObject data)
    {
        ...
        { //await tổng bao nhiêu lâu thì HandleAnyoneReceivesLuckyCards() phải await bấy nhiêu tránh xung đột tween rotate
            ...
            await Awaitable.WaitForSecondsAsync(CARD_FLYING_DURATION);
            PlayerViewLucky89 myPlayerview = (PlayerViewLucky89)thisPlayer.playerView;
            ...
            _SetTickDraw(true, 0);
            myPlayerview.ShowRate((int)data["rate"]).ShowScore(true, myScore);
        }
    }
    private void _HandleAnyoneReceivesLuckyCards(JObject data)
    {
        ...
        async Awaitable handleData()
        {
            ...
            {
                Player player = players.Find(x => x.namePl.Equals((string)dataLucky["N"]));
                ...
                for (int i = 0; i < dataCards.Count; i++) _RevealACard(cardCs[i], (int)dataCards[i], cardCs[i].transform.localRotation.eulerAngles);
                playerView.ShowRate((int)dataLucky["rate"]).ShowScore(true, (int)dataLucky["score"]);
            }
        }
    }
    private void _HandleAnyoneTimeOut(JObject data)
    {
        ...
        async Awaitable handleData()
        {
            ...
            if (player != null)
            {
                ...
                {
                    if (_drawACard) SocketSend.SendDrawACardLucky89(1);
                    _isMyDrawTime = true;
    ...
    private void _RevealACard(Card cardC, int cardCode, Vector3 targetRotV3)
    {
        ...
        else
        {
            ...
            cardC.transform.DOLocalRotate(new Vector3(0, 90, 0) + targetRotV3, CARD_ROTATING_DURATION).SetEase(Ease.InQuad).OnComplete(() =>
            {
                ...
                cardC.transform.DOLocalRotate(targetRotV3, CARD_ROTATING_DURATION).SetEase(Ease.OutQuad);
            });
        }
    }
    ...
    private async Awaitable _DrawCard(PlayerViewLucky89 playerView, int cardCode = 0)
    {
        ...
        Vector3 targetRotV3 = cardRT.localRotation.eulerAngles;
