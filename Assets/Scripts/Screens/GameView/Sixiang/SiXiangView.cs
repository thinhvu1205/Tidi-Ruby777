...
public class SiXiangView : BaseSlotView
{
            ...
            case (int)GAME_TYPE.RAPID_PAY:
                {
                    ...
                    await showAnimCutScene();
                    await showRapidPayGame(validBetLevels[currentBetLevel]);
                    break;
                }
            ...
            case (int)GAME_TYPE.RAPID_PAY:
                {
                    ...
                    await showRapidPayGame(winAmount, rewards);
                    break;
                }
            case (int)GAME_TYPE.LUCKY_DRAW:
                {
    ...
    public override void handleBonusInfo(JObject data)
    {
        ...
        bonusGame.ForEach((bonus) =>
        {
            ...
            DOTween.Kill(btnBuyPeals[indexPeal].transform);
            sprPeals[indexPeal].material = materialPeals[0];
            btnBuyPeals[indexPeal].transform.DOLocalMoveX(-5, 0.5f).SetEase(Ease.OutSine).OnComplete(() =>
            {
                btnBuyPeals[indexPeal].gameObject.SetActive(false);
            });
        });
        if (gameType != 0)
        {
    ...
    private void setStateButtonBuyPeals(bool state)
    {
        if (!state)
        {
            for (int i = 0; i < 4; i++)
            {
                Transform btnPeal = btnBuyPeals[i].transform;
                DOTween.Kill(btnPeal);
                btnPeal.DOLocalMoveX(-5, 0.3f).SetEase(Ease.OutSine).OnComplete(() =>
                {
                    btnPeal.gameObject.SetActive(false);
                });
            }
        }
        else  //phai check nhung thang nao dang duoc active de bam mua duoc.
        {
            ...
            bonusGame.ForEach((bonus) =>
            {
                listIndexPealActive.Add(getIndexPeals(bonus));
            });
            for (int i = 0; i < 4; i++)
            {
                Transform btnPeal = btnBuyPeals[i].transform;
                if (listIndexPealActive.Contains(i))
                { //an di
                    btnPeal.DOLocalMoveX(-5, 0.3f).SetEase(Ease.OutSine).OnComplete(() =>
                    {
                        btnPeal.gameObject.SetActive(false);
                ...
                {
                    btnPeal.transform.DOLocalMoveX(75, 0.3f).SetEase(Ease.OutBack).SetDelay(0.05f * i);
                    btnPeal.gameObject.SetActive(true);
                }
            }
        }
    }
    ...
    public override void onSpinTriggerUp()
    {
        ...
        {
            Debug.Log("onSpinTriggerUp DP --> Game State==" + gameState);
    ...
    public async Task endMinigame(JObject data)
    {
        ...
        if (gameType == (int)GAME_TYPE.DRAGON_PEARL)
        {
            ...
            await showResultMoneyAnim(PATH_ANIM_WINRESULT_DP, "cam", winAmount, new Vector2(0, -68));
            activeAllSymbol();
        }
        else
        {
    ...
    protected virtual async Task showLuckyGoldView(int remainPick = 20)
    {
        ...
        if (LuckyGoldView == null)
        {
            ...
            LuckyGoldView.transform.SetSiblingIndex(animCutScene.transform.GetSiblingIndex() - 1);
        }
    ...
    protected virtual async Task showRapidPayGame(int initWinAmount, List<JObject> data = null)
    {
        ...
        if (RapidPayView == null)
        {
            ...
            RapidPayView.transform.SetSiblingIndex(animCutScene.transform.GetSiblingIndex() - 1);
        }
        RapidPayView.winAmount = initWinAmount;
        await RapidPayView.Show(this, data);
    }
