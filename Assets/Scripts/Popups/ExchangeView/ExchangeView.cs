...
public class ExchangeView : BaseView
{
    ...
    [SerializeField] GameObject tabLeft, tabTop, itemEx, itemAgency, itemHistory;
    ...
    [SerializeField] TMP_InputField ifPhone, ifPhoneRetype;
    ...
    async Task genTabTop(JArray arrayData)
    {
        ...
        for (var i = 0; i < arrayData.Count; i++)
        {
            ...
            GameObject btn = Instantiate(tabTop);
    ...
    private async void genTabHis(JArray arrayData)
    {
        ...
        for (var i = 0; i < arrayData.Count; i++)
        {
            ...
            btn.transform.SetParent(scrTabsHis.content);
            btn.transform.localScale = Vector3.one;
            btn.transform.position = new Vector3(btn.transform.position.x, 0);
    ...
    void reloadListItem(JObject objDataItem)
    {
        if (objDataItem != null)
        {
            ...
            for (var i = 0; i < items.Count; i++)
            {
                ...
                GameObject item = i < parent.childCount ? parent.GetChild(i).gameObject : Instantiate(isAgency ? itemAgency : itemEx);
    ...
    public void reloadListItemHistory(List<JObject> listItem)
    {
        ...
        for (var i = 0; i < listDataHis.Count; i++)
        {
            ...
            if (typeNameItem == typeTabHistory)
            {
                GameObject objItem;
    ...
    public void onConfirmCashOut()
    {
        ...
        if (rewardData != null)
        {
            JArray textBox = (JArray)rewardData["child"][indexTabNap]["textBox"];
            ifPhone.placeholder.GetComponent<TextMeshProUGUI>().text = Globals.Config.getTextConfig((string)textBox[0]["key_placeHolder"]);
            ifPhoneRetype.placeholder.GetComponent<TextMeshProUGUI>().text = Globals.Config.getTextConfig((string)textBox[1]["key_placeHolder"]);
            //Debug.Log("rewardData:"+ rewardData.ToString());
        }
