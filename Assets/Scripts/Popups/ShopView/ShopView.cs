...
public class ShopView : BaseView
{
    ...
    async void updateInfo(string strData)
    {
        ...
        for (var i = 0; i < arrayData.Count; i++)
        {
            ...
            if (obItem.ContainsKey("focus") && ((bool)obItem["focus"]) == true)
            {
                indSelect = i;
                item0 = obItem;
            }
            ...
            if (title.Equals("iap") && iapManager == null)
            {
        ...
        {
            onClickTab(scrTabs.content.transform.GetChild(indSelect).gameObject, item0);
        }
    }
